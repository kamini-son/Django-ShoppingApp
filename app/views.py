...
def mobile(request, data=None):
    ...
    if data is None:
        mobiles = Product.objects.filter(category='M')
    elif data in ['Apple Iphone', 'Samsung']:
        ...
        # Handle cases where data is not one of the expected values
        mobiles = Product.objects.none()  # Or set to an empty queryset
...
def laptop(request, data=None):
    ...
    if data is None:
        laptops = Product.objects.filter(category='L')
    elif data in ['Apple', 'Lenovo', 'DELL', 'HP']:
        ...
        laptops = Product.objects.none()  # Handle invalid filter options
...
def custom_logout_view(request):
    if request.method == "POST":
        logout(request)
        return redirect('home')  # Redirect to home page after successful logout
    return redirect('home')  # Prevent GET requests